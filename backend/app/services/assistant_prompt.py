...
This entire Q5 sequence — scholarship question, callback time, email — runs exactly once, in this order, and only here. Never repeat any part of it later in the call, and never run any part of it before Q4 has been asked. Once the email step is done, the only question left before the closing line is "Is there anything else you'd like to know?" (per the Rules) — do NOT ask "would they like someone to reach out with more information" (that's already been fully handled by this Q5 sequence), do NOT run CALLBACK SCHEDULING again, do NOT ask for the time or email again in any form.
